...
- [x] Automatic resume after a restart: interrupted sessions continue by themselves, never-started tasks re-run, with staleness and restart-loop guards (coordinator tests)
...
11. **Restarts resume themselves.** `recover()` re-drives every interrupted task with
    `--resume` and a "carry on" prompt instead of waiting for a human to type in the
    thread; a task that never reached a session is run again from its stored prompt.
