...
| `/model opus`, `/clear`, `/compact`, … | the agent's *own* commands, passed through to it as typed — every one its CLI, plugins and the project define, including ones added after this table. A chat app that runs a command name itself (Slack's `/rename`) never delivers it; address the bot first for those (`@dispatch /rename`) |
