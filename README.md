...
| `merge`                      | get this thread's pull request merged, then close the thread. The agent commits and pushes whatever is outstanding first, and resolves a conflict with the base branch if GitHub reports one; then dispatch runs the merge itself (`gh pr merge --squash --delete-branch` with the host's login), so the thread closes on GitHub's exit code, not on an agent's account of it. A red check, a missing approval or a branch protection rule is reported, not worked around — the thread stays open with gh's reason. `merge rebase` / `merge merge` to not squash |
