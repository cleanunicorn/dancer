...
| `/model opus`, `/clear`, `/compact`, … | the agent's *own* commands, passed through to it as typed — every one its CLI, plugins and the project define, including ones added after this table. On Slack write `@dancer /clear`: Slack keeps a message that *starts* with `/` for itself |
